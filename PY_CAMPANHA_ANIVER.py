...
from functions.data_base import db_connection #  função para conectar no banco de dados
...
try:
    with db_connection() as connection:  # conectando com banco de dados pela função da pasta functions
        with connection.cursor() as cursor:  # abrindo central para rodar o comando query
            ...
            # Definindo o caminho em que o arquivo deve ser salvo
            folder = 'H:/Tecnologia/EQUIPE - DADOS/6 - Voip/1 - Aniversariantes'

            # Verificando se a pasta existe, caso contrário, criar a pasta
            if not os.path.exists(folder):
                os.makedirs(folder)

            # Definindo o nome do arquivo e o local do salvamento
            csv_file = os.path.join(folder, f'{year}_{month}_{day}.csv')
